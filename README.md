...
This repository is an executable Drosera-compatible PoC for `HUNDRED_EMPTY_MARKET_EXCHANGE_RATE`.
...
Hundred Finance-style empty-market donation exploits inflate exchange rate when supply is near empty. The dangerous transition is low token supply, increased cash, and exchange-rate growth that is not justified by normal borrow/reserve movement. The mock stages donation-driven rate inflation, then attempts to borrow against the inflated rate.
...
- `src/HundredEmptyMarketRateTrap.sol`
- `src/HundredEmptyMarketRateResponse.sol`
...
```text
HUNDRED_EMPTY_MARKET_EXCHANGE_RATE_V2
```

Response action:

```text
pause mint, borrow, redeem, and liquidate
```
...
Also confirm the Drosera executor caller in `src/HundredEmptyMarketRateResponse.sol`:
